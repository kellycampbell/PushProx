...
var (
	...
	logFormat = flag.String("log.format", "logger:stderr", "URL path for surfacing collected metrics")
)
...
func InitLogger() {
	err := baseLogger.SetLevel(*logLevel)
	if err != nil {
		fmt.Println("Error setting log level")
	}
	err = baseLogger.SetFormat(*logFormat)
	if err != nil {
		fmt.Println("Error setting log format")
	}
}
